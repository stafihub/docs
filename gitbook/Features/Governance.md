...
- **Vote:** Once deposit reaches a certain value (`MinDeposit`), proposal is
  confirmed and vote opens. Bonded FIS holders can then send `TxGovVote`
...
#### Right to submit a proposal*__*

Any FIS holder, whether bonded or unbonded, can submit proposals by sending a
...
Cosmos Hub, participants are bonded FIS holders. Unbonded FIS holders and
...
- _participant_ bonded or unbonded FIS to said validator after proposal
  entered voting period.

- _participant_ became validator after proposal entered voting period.

This does not prevent _participant_ to vote with FIS bonded to other
validators. For example, if a _participant_ bonded some FIS to validator A
before a proposal entered voting period and other FIS to validator B after
...
Proposals can be accepted before the end of the voting period if they meet a special condition. Namely, if the ratio of `Yes` votes to `InitTotalVotingPower`exceeds 2:3, the proposal will be immediately accepted, even if the `Voting period` is not finished. `InitTotalVotingPower` is the total voting power of all bonded FIS holders at the moment when the vote opens.
...
stafihubd query service params
...
stafihub tx gov submit-proposal param-change proposal.json --from=<key-name> --fees=0.3fis --chain-id=stafihub
```
...
stafihub tx gov submit-proposal community-pool-spend proposal.json --from=<key-name> --fees=0.3fis --chain-id=stafihub
```
